...
impl Job<AppState> for UpdateProfilePictureProgressJob {
    ...
    async fn run(&self, app_state: AppState) -> cja::Result<()> {
        ...
        let progress = match crate::profile_progress::ProfilePictureProgress::get_by_token_id(
            ...
            None => {
                error!("No progress settings found for token ID {} in job", self.token_id);
        ...
        if !progress.enabled {
            debug!("Profile picture progress feature is disabled for token ID {}", self.token_id);
            return Ok(());
        }
        ...
        let (numerator, denominator) = match &token.handle {
            Some(handle) => {
                extract_progress_from_handle(handle).unwrap_or((0.0, 1.0))
            },
        ...
        debug!("Progress for token {}: {}/{} = {:.2}%", 
               self.token_id, numerator, denominator, progress_percentage * 100.0);

        // Fetch the original profile picture
        let original_image_data = match crate::routes::bsky::fetch_blob_by_cid(&token.did, &original_blob_cid).await {
            Ok(data) => data,
            Err(err) => {
        ...
        let progress_image_data = match generate_progress_image(&original_image_data, progress_percentage).await {
            Ok(data) => {
                info!("Successfully generated progress image for token ID {}", self.token_id);
                data
            },
            Err(err) => {
        ...
        match upload_image_to_bluesky(&app_state, &token, &progress_image_data).await {
            Ok(cid) => {
                info!("Successfully uploaded progress image to Bluesky with CID: {}", cid);
                
                // Update profile with the new image
                match update_profile_with_image(&app_state, &token, &cid).await {
                    Ok(_) => {
                        info!("Successfully updated profile with progress image for token ID {}", self.token_id);
                    },
                    Err(err) => {
...
async fn generate_progress_image(original_image_data: &[u8], progress: f64) -> cja::Result<Vec<u8>> {
    use color_eyre::eyre::eyre;
    use image::{ImageFormat, Rgba};
    use imageproc::drawing::{draw_hollow_circle_mut, draw_filled_circle_mut, draw_filled_rect_mut};
    ...
    let bottom_rect = Rect::at(0, (height - overlay_height) as i32)
        .of_size(width, overlay_height);
    ...
    // Draw outline circle
    draw_hollow_circle_mut(&mut img, (center_x as i32, center_y as i32), radius, outline_color);
...
async fn upload_image_to_bluesky(
    ...
    match blob_cid {
        ...
        None => {
            error!("Failed to extract blob CID from response: {:?}", response_data);
...
async fn update_profile_with_image(
    ...
    if let serde_json::Value::Object(ref mut obj) = profile_value {
        ...
        obj.insert("avatar".to_string(), serde_json::json!({
            "$type": "blob",
            "ref": {
                "$link": blob_cid
            },
    ...
    if !put_response.status().is_success() {
        ...
        return Err(eyre!("Failed to update profile: {} - {}", status, error_text));
    }
    
    info!("Successfully updated profile with new image for DID: {}", token.did);
    Ok(())
}
